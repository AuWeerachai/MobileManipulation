...
Role in the three-person pipeline:
  - Friend 1 (perception): publishes geometry_msgs/PoseArray on /nav/goals,
    containing up to 4 target objects (x/y in the map frame). Orientation is
    ignored; this node computes a yaw that faces each goal.
  - Friend 2 (manipulation): publishes std_msgs/String on /nav/control_flag
    with payload "proceed" (go / advance to next goal) or "stop" (abort).
  - This node drives with Nav2 (via stretch_nav2.BasicNavigator, the same
    helper used in Lab4/nav2_stretch.py). When the robot is within
    STOP_DISTANCE_M of the current goal, Nav2 is cancelled and this node
    publishes std_msgs/String "arrived" on /nav/arrived_flag so manipulation
    can take over with its IK + grasp flow.
...
Transitions:
  any + PoseArray on /nav/goals        -> AWAITING_GO, index = 0, cancel any
                                          in-flight Nav2 task.
  AWAITING_GO + "proceed"              -> NAVIGATING (send goals[i])
  NAVIGATING  + distance <= STOP_M     -> HANDOFF (cancel Nav2, publish
                                          "arrived", i += 1)
  HANDOFF     + "proceed"              -> NAVIGATING (send goals[i]) or
                                          IDLE if all goals done.
  any         + "stop"                 -> cancel Nav2, revert to AWAITING_GO on
                                          current index (so next "proceed"
                                          retries the same goal).

Assumptions:
  - Nav2 is already running against a saved map (e.g., the copied
    asangium.yaml). Bring it up with the stretch_nav2 navigation launch file
...
# off to manipulation. 1.0 m is the spec from the team plan.
STOP_DISTANCE_M = 1.0
...
CONTROL_TOPIC = "/nav/control_flag"   # friend2 -> me   (String: "proceed"/"stop")
ARRIVED_TOPIC = "/nav/arrived_flag"   # me -> friend2   (String: "arrived")
...
class NavigationCoordinator(Node):
    """Coordinator node wired between perception, manipulation, and Nav2."""
    ...
    def _control_callback(self, msg: String) -> None:
        """React to proceed / stop from manipulation."""
    ...
    def _timer_callback(self) -> None:
        ...
        if self.navigator.isTaskComplete():
            ...
            self.get_logger().info(
                f"Nav2 task completed with result={result} before 1m stand-off; handing off."
            )
            self._handoff()
            return
