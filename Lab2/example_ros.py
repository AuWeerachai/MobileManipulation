...
class SangStretchNode(hm.HelloNode):
    ...
    def get_current_configuration(self):
        ...
        q_base = 0.0
        q_lift = bound_range('joint_lift', self.joint_pos['joint_lift'])
        q_arml = bound_range('joint_arm_l0', self.joint_pos['joint_arm'] / 4.0)
        q_yaw = bound_range('joint_wrist_yaw', self.joint_pos['joint_wrist_yaw'])
        q_pitch = bound_range('joint_wrist_pitch', self.joint_pos['joint_wrist_pitch'])
        q_roll = bound_range('joint_wrist_roll', self.joint_pos['joint_wrist_roll'])
    ...
    def move_to_configuration(self,q):
        ...
        self.move_to_pose({'translate_mobile_base': float(q_base)}, blocking=True)
        self.move_to_pose({'joint_lift': float(q_lift)}, blocking=True)
        self.move_to_pose({'joint_arm': float(q_arm)}, blocking=True)
        self.move_to_pose({'joint_wrist_yaw': float(q_yaw)}, blocking=True)
        self.move_to_pose({'joint_wrist_pitch': float(q_pitch)}, blocking=True)
        self.move_to_pose({'joint_wrist_roll': float(q_roll)}, blocking=True)
    ...
    def main(self):
        ...
        def give_jointstate(msg):
            for n, p in zip(msg.name, msg.position):
                self.joint_pos[n] = p

        self.create_subscription(JointState, "/joint_states", give_jointstate, 10)

        t = time.time()
        while rclpy.ok() and time.time() - t < 5.0:
            rclpy.spin_once(self, timeout_sec=0.1)
            if 'joint_lift' in self.joint_pos:
                break
