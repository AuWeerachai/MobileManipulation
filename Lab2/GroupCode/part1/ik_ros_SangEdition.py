...
class StretchIKDemo(hm.HelloNode):
    ...
    def setup_ik_chain(self): #Setup the IK chain from URDF with virtual base joint.
        ...
        # amend the chain (reconnect the new whole thing between mast and base)
        for j in modified_urdf._joints:
            if j.name == 'joint_mast':
                j.parent = 'link_base_translation'     #joint_mast -> link_base_translation -> joint_base_translation -> link_base_rotation -> joint_base_rotation -> base_link
        ...
        #turn off calculation on some links those are fixed
        active_links_mask = [
        ...
        #get the chain and return it from saved modified
    ...
    def move_to_grasp_goal(self, target_point, target_orientation):
        ...
        #check if answer is acceptable, *if not none, if return something it means IK works and get us close to config. If found any problems in case that solver can get answer, probably from something else
